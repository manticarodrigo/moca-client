      ...
      {sessionHistory.map((session, index) => (
        ...
        <View key={index} row variant="borderBottom" spacing={{ py: 3 }}>
          <View column justifyCenter variant="borderRight" spacing={{ p: 2 }} width={90}>
            ...
            <Text typography={{ size: 2, color: 'secondary', weight: '500' }}>{session.month} / {session.year}</Text>
          </View>
          <View column justifyCenter variant="borderRight" spacing={{ p: 2 }} width={250}>
          ...
          <View justifyCenter spacing={{ p: 2 }} width={80}>
            <Text variant="titlePrimaryLarge">{session.payment}</Text>
          </View>
        </View>
      ))}
    </View>
